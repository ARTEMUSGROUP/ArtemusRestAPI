...
public class JpLocationDAO {
	...
	public JpLocationDAO(Connection connection) {
		if (connection != null) {
			con = connection;
		} else {
	...
	public String getLocationCode(String Unlocode, String loginScac) {
		...
		try {
			...
			if (rs.next()) {
				...
				stmt2 = con.prepareStatement("SELECT location_code FROM location where unlocode=?");
	...
	public Boolean isForeignPort(String unCode) {
		...
		Boolean result = true;
		try {
			stmt = con.prepareStatement("select port_name, port_code from district_port where port_code=? " + " union"
	...
	public boolean isDisctrictPort(String unlocode) {
		...
		Boolean result = true;
		try {
			stmt = con.prepareStatement("select port_name, port_code from japan_port where port_code=? ");
			stmt.setString(1, unlocode);
			rs = stmt.executeQuery();
			logger.info(stmt);
			if (rs.next()) {
	...
	public int getLocationIdfromUnlocode(String Custcode, String loginScac) {
		...
		int result = 1;
		try {
			stmt2 = con.prepareStatement("SELECT location_id FROM location where location_code=? and login_scac=? and location_code!=''");
